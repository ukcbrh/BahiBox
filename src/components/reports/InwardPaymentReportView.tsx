...
export function InwardPaymentReportView({ onBack }: { onBack?: () => void }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      toast.error(error.message || 'Failed to fetch inward payment report');
    } finally {
      setLoading(false);
    }
  };
  ...
  const reportRows = payments.map((p: any) => ({
    'Date': p.payment_date,
    'Customer': customerMap[p.customer_id] || 'Unknown',
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Inward Payment Report</h2>
          <p className="text-slate-500 dark:text-slate-400">All customer payments collected in the selected period.</p>
        </div>
        <ReportActionButtons
          data={reportRows}
          columns={reportColumns}
          filename={`inward_payment_report_${startDate}_to_${endDate}`}
          title="Inward Payment Report"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="border-none shadow-sm bg-gradient-to-br from-emerald-50 to-emerald-100/50">
          <CardContent className="p-5">
            ...
            <p className="text-3xl font-bold text-emerald-800 mt-1">₹{totalCollected.toLocaleString('en-IN')}</p>
            <p className="text-xs text-emerald-600 mt-1">{payments.length} payment{payments.length !== 1 ? 's' : ''}</p>
          </CardContent>
        </Card>
        <Card className="border-none shadow-sm">
          <CardContent className="p-5">
                ...
                payments.map((p: any) => (
                  <tr key={p.id} className="border-b hover:bg-slate-50 dark:hover:bg-slate-900/50">
                    ...
                    <td className="px-6 py-4 font-medium text-slate-900 dark:text-slate-100">
                      <div className="flex items-center"><Wallet className="h-4 w-4 mr-2 text-slate-400" />{customerMap[p.customer_id] || 'Unknown'}</div>
                    </td>
