  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">Onboard your bank account to receive payments directly via Razorpay Route.</p>
      </div>
        ...
        <CardContent className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-2 border-b pb-2">
                <Building size={16} /> Business Details
              </h3>
              <div className="space-y-3">
                <div className="space-y-1">
                  <label className="text-xs font-medium text-slate-500 dark:text-slate-400">Legal Business Name</label>
                  <Input value="Sharma General Store" disabled={kycStatus !== 'pending'} className="bg-slate-50 dark:bg-slate-900" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-medium text-slate-500 dark:text-slate-400">Business PAN</label>
                  <Input value="ABCDE1234F" disabled={kycStatus !== 'pending'} className="bg-slate-50 dark:bg-slate-900 font-mono" />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="space-y-1">
                  ...
                  <Input value={kycStatus === 'active' ? "•••• •••• 9876" : ""} disabled={kycStatus !== 'pending'} className="bg-slate-50 dark:bg-slate-900 font-mono" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-medium text-slate-500 dark:text-slate-400">IFSC Code</label>
                  <Input value={kycStatus === 'active' ? "SBIN0001234" : ""} disabled={kycStatus !== 'pending'} className="bg-slate-50 dark:bg-slate-900 font-mono" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-medium text-slate-500 dark:text-slate-400">Account Holder Name</label>
                  <Input value="Sharma General Store" disabled={kycStatus !== 'pending'} className="bg-slate-50 dark:bg-slate-900" />
                </div>
              </div>
            </div>
          ...
          {kycStatus === 'active' && (
            <div className="mt-6 pt-6 border-t flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">Automatically transfer funds to your bank account daily.</p>
              </div>
      ...
      <Card className="border-none shadow-sm">
        ...
        {useOwnGateway && (
          <CardContent className="p-6 space-y-4 bg-slate-50 dark:bg-slate-900/50">
