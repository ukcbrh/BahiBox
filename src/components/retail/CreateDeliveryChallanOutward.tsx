...
interface CreateDeliveryChallanOutwardProps {
  onBack: () => void;
  editData?: any;
}
...
export function CreateDeliveryChallanOutward({ onBack, editData }: CreateDeliveryChallanOutwardProps) {
  ...
  useEffect(() => {
    if (!editData) {
      try {
        ...
        setInvoicePrefix(settings?.['delivery_challan_out']?.prefix || 'DCO-');
      } catch (e) {
        setInvoicePrefix('DCO-');
      }
      setInvoiceNumberPart(Date.now().toString().slice(-8));
    }
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const challanGrandTotal = items.reduce((s: number, it: any) => {
      ...
      // it links already-recorded advance money to this new challan.
      ...
      if (requestedAdvance > 0 && selectedCustomer.balance_label === 'Advance') {
        const { data: allocated, error: allocErr } = await supabase.rpc('allocate_advance_to_bill', {
          ...
          p_bill_type: 'delivery_challan_outward',
          p_bill_id: challanId,
        ...
        if (allocErr) {
          ...
          toast.error('Delivery Challan created, but advance could not be applied: ' + allocErr.message);
        } else {
          ...
          if (token) {
            const res = await fetch('/api/create-invoice-payment-qr', {
              ...
              body: JSON.stringify({
                ...
                amount: challanGrandTotal - actualAdvanceApplied,
                due_days: 30
              })
            });
            if (res.ok) {
      ...
      const billData: BillData = {
        ...
        totals: {
          ...
          tax_breakdown: actualAdvanceApplied > 0 ? [
            { label: 'Bill Amount', amount: challanGrandTotal },
            { label: 'Less: Advance Applied', amount: actualAdvanceApplied }
          ] : [],
          grand_total: challanGrandTotal - actualAdvanceApplied
        },
        footer: {
          stamp_url: brandingInfo?.stamp_url
        },
        ...
        upi_payment: (paymentMethod === 'credit' && brandingInfo?.show_upi_qr_on_credit)
          ? (useRazorpayQr
              ? (qrImageUrl ? { qr_image_url: qrImageUrl, amount: challanGrandTotal - actualAdvanceApplied } : undefined)
              : (branchInfo?.upi_id ? { upi_id: branchInfo.upi_id, payee_name: tenantData?.business_name || 'Merchant', amount: challanGrandTotal - actualAdvanceApplied } : undefined))
          : undefined
      };
      ...
      toast.success(actualAdvanceApplied > 0 ? `Delivery Challan created — ₹${actualAdvanceApplied.toFixed(2)} advance applied` : 'Delivery Challan created successfully');
    } catch (err: any) {
  ...
  if (successInvoice) {
    return (
      <div className="max-w-lg mx-auto py-16 text-center space-y-4">
        ...
        <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200">Delivery Challan Created!</h2>
        <p className="text-slate-500 dark:text-slate-400">{successInvoice.challan_number}</p>
        <p className="text-3xl font-bold text-slate-900 dark:text-white">₹{totals.grand.toFixed(2)}</p>
        <p className="text-sm text-slate-500 dark:text-slate-400">Status: {successInvoice.payment_status || 'unpaid'}</p>
    ...
    <div className="space-y-4 max-w-[1400px] mx-auto animate-in fade-in duration-300">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300">
          ...
          <h2 className="text-xl font-semibold">Delivery Challan (Outward)</h2>
        </div>
      ...
      <Card className="shadow-sm border-slate-200 dark:border-slate-800 h-full">
        <CardHeader className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
          <CardTitle className="text-sm font-semibold text-slate-700 dark:text-slate-300">DC Out Detail</CardTitle>
        </CardHeader>
          <CardContent className="p-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid grid-cols-[100px_1fr] items-center gap-2">
                <label className="text-sm text-slate-600 dark:text-slate-400 whitespace-nowrap">DC Out Number<span className="text-red-500">*</span></label>
