...
interface CreateDeliveryChallanInwardProps {
  onBack: () => void;
  editData?: any;
}

export function CreateDeliveryChallanInward({ onBack, editData }: CreateDeliveryChallanInwardProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const challanGrandTotal = items.reduce((sum: number, item: any) => {
        const taxRate = item.tax_rate || 0;
        const baseRate = item.p_tax ? (item.price / (1 + taxRate / 100)) : item.price;
      ...
      // this supplier extra) to this new challan, if the user chose to. 
      // This does NOT create a new payment/journal entry — it links 
      // already-recorded advance money to this challan for tracking.
      ...
      if (requestedAdvance > 0 && selectedSupplier?.balance_label === 'Advance') {
        const { data: allocated, error: allocErr } = await supabase.rpc('allocate_advance_to_bill', {
          ...
          p_bill_type: 'delivery_challan_inward',
          p_bill_id: challanId,
        ...
        if (allocErr) {
          ...
          toast.error('Delivery Challan saved, but advance could not be applied: ' + allocErr.message);
        } else {
      ...
      await printBillForChannel(supabase, currentTenantId as any, 'delivery_challan_in', billData, 'A4');

      toast.success(actualAdvanceApplied > 0 ? `Delivery Challan saved — ₹${actualAdvanceApplied.toFixed(2)} advance applied` : 'Delivery Challan saved successfully');
      onBack();
    ...
    <div className="space-y-4 max-w-[1400px] mx-auto animate-in fade-in duration-300 pb-12">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300">
          ...
          <h2 className="text-xl font-semibold">Create Delivery Challan (Inward)</h2>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="shadow-sm border-slate-200 dark:border-slate-800 h-full">
          <CardHeader className="p-4 border-b border-slate-100 dark:border-slate-800 flex flex-row items-center justify-between bg-slate-50 dark:bg-slate-900/50">
            <CardTitle className="text-sm font-semibold text-slate-700 dark:text-slate-300">DC In Detail</CardTitle>
          </CardHeader>
          <CardContent className="p-4 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="grid grid-cols-[100px_1fr] items-center gap-2">
                <label className="text-sm text-slate-600 dark:text-slate-400 whitespace-nowrap">DC In Number<span className="text-red-500">*</span></label>
                <Input 
                  className="h-9 w-full" 
