...
export function SaleOrderForm({ onBack, editSaleOrder }: SaleOrderFormProps & { editSaleOrder?: any }) {
  ...
  const [visibleColumns, setVisibleColumns] = useState(() => {
    const saved = localStorage.getItem(tenantScopedKey('sale_order_columns', currentTenantId));
    if (saved) {
  ...
  useEffect(() => {
    localStorage.setItem(tenantScopedKey('sale_order_columns', currentTenantId), JSON.stringify(visibleColumns));
  }, [visibleColumns]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await supabase.from('sale_orders').update({ sale_order_number: invoiceNo || undefined }).eq('id', saleOrderId);
      const { data: saleOrderRow } = await supabase.from('sale_orders').select('*').eq('id', saleOrderId).single();
      ...
      const savedSettingsStr = localStorage.getItem(tenantScopedKey('posSettings', currentTenantId));
      const savedSettings = savedSettingsStr ? JSON.parse(savedSettingsStr) : null;
      const printerSize = savedSettings?.['sale_order']?.printerSize || 'A4';
      ...
      const billData: BillData = {
        ...
        meta: { label: 'Sale Order', number: invoiceNo || 'SO-0001', date: new Date().toLocaleDateString() },
      ...
      setSuccessInvoice(saleOrderRow);
  ...
  if (successInvoice) {
    return (
      <div className="max-w-lg mx-auto py-16 text-center space-y-4">
        ...
        <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200">Sale Order Created!</h2>
