...
export function QuotationForm({ onBack, editQuotation }: QuotationFormProps & { editQuotation?: any }) {
  ...
  const [visibleColumns, setVisibleColumns] = useState(() => {
    const saved = localStorage.getItem(tenantScopedKey('quotation_columns', currentTenantId));
    if (saved) {
  ...
  useEffect(() => {
    localStorage.setItem(tenantScopedKey('quotation_columns', currentTenantId), JSON.stringify(visibleColumns));
  }, [visibleColumns]);
  ...
  useEffect(() => {
    const generateDefaultInvoiceNo = async () => {
      ...
      const prefix = savedSettings?.['quotation']?.prefix || 'QTN-';
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const savedSettingsStr = localStorage.getItem(tenantScopedKey('posSettings', currentTenantId));
      const savedSettings = savedSettingsStr ? JSON.parse(savedSettingsStr) : null;
      const printerSize = savedSettings?.['quotation']?.printerSize || 'A4';
      ...
      const billData: BillData = {
        ...
        meta: { label: 'Quotation', number: invoiceNo || 'QTN-0001', date: new Date().toLocaleDateString() },
  ...
  if (successInvoice) {
    return (
      <div className="max-w-lg mx-auto py-16 text-center space-y-4">
        ...
        <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200">Quotation Created!</h2>
