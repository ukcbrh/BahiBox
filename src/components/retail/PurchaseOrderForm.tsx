...
export function PurchaseOrderForm({ onBack, editData: editPO }: CreatePurchaseInvoiceProps) {
  ...
  useEffect(() => {
    if (!editPO || suppliers.length === 0) return;
    setSupplierId(editPO.supplier_id);
    const sup = suppliers.find((s: any) => s.id === editPO.supplier_id);
    if (sup) {
      setSelectedSupplier(sup);
      setSupplierSearchTerm(sup.supplier_name || '');
    }
    if (editPO.po_number) setInvoiceNo(editPO.po_number);
    const editedItems = (editPO.purchase_order_document_items || []).map((it: any) => ({
  ...
  const handleSave = async () => {
    if (!supplierId || !invoiceNo || items.length === 0) {
      toast.error('Please fill supplier, PO number and add at least one item');
      return;
    }
    ...
    try {
      ...
      let poId: string;
      if (editPO) {
        const { error: updErr } = await supabase.rpc('update_purchase_order_document', {
          p_purchase_order_document_id: editPO.id,
          p_supplier_id: supplierId,
          p_supplier_name: suppliers.find((s: any) => s.id === supplierId)?.supplier_name || null,
          p_supplier_gstin: suppliers.find((s: any) => s.id === supplierId)?.gstin || null,
        ...
        const foundSup = suppliers.find((s: any) => s.id === supplierId);
        const { data: newId, error } = await supabase.rpc('create_purchase_order_document', {
          p_tenant_id: currentTenantId,
          p_branch_id: branchId,
          p_created_by: user?.id,
          p_supplier_id: supplierId,
          p_supplier_name: foundSup?.supplier_name || null,
          p_supplier_gstin: foundSup?.gstin || null,
          p_expected_delivery_date: null,
          p_notes: null,
          p_items: rpcItems
        });
      ...
      await supabase.from('purchase_order_documents').update({ po_number: invoiceNo || undefined }).eq('id', poId);
      const { data: poRow } = await supabase.from('purchase_order_documents').select('*').eq('id', poId).single();
      ...
      const billData: BillData = {
        ...
        meta: { label: 'Purchase Order', number: invoiceNo, date: new Date(invoiceDate).toLocaleDateString() },
      ...
      await printBillForChannel(supabase, currentTenantId as any, 'purchase_order', billData, 'A4');
          ...
          <CardContent className="p-4 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="grid grid-cols-[100px_1fr] items-center gap-2">
                <label className="text-sm text-slate-600 dark:text-slate-400 whitespace-nowrap">PO No.<span className="text-red-500">*</span></label>
                <Input 
                  className="h-9 w-full" 
