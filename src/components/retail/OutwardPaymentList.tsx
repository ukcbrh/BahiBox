...
import { CreateOutwardPayment } from './CreateOutwardPayment';
...
export function OutwardPaymentList() {
  ...
  const fetchHistory = async () => {
    ...
    const { data } = await supabase
      ...
      .select('*, suppliers(supplier_name, contact_phone, address), purchase_invoices(invoice_number), delivery_challans_inward(challan_number)')
  ...
  if (isCreating) {
    return <CreateOutwardPayment onBack={() => setIsCreating(false)} />;
  }

  if (editingGroup) {
    return <CreateOutwardPayment onBack={() => setEditingGroup(null)} editGroup={editingGroup} />;
  }
  ...
  paymentHistory.forEach((p: any) => {
    ...
    if (!groupsMap.has(key)) {
      groupsMap.set(key, {
        ...
        partyName: p.suppliers?.supplier_name || 'Unknown',
  ...
  const handleViewPrint = async (group: ReceiptGroup) => {
    ...
    try {
      ...
      const receiptItems = group.rows.map((p: any) => ({
        name: p.purchase_invoices?.invoice_number
          ? 'Applied to Invoice ' + p.purchase_invoices.invoice_number
          : p.delivery_challans_inward?.challan_number
          ? 'Applied to Delivery Challan ' + p.delivery_challans_inward.challan_number
          : 'On Account / Advance',
        qty: 1,
      ...
      const billData: BillData = {
        ...
        customer: { name: group.partyName, phone: firstRow?.suppliers?.contact_phone, address: firstRow?.suppliers?.address },
  ...
  const handleDelete = async (group: ReceiptGroup) => {
    ...
    try {
      for (const row of group.rows) {
        const { error } = await supabase.rpc('delete_supplier_payment', { p_payment_id: row.id });
    ...
    <div className="p-6 md:p-8 max-w-5xl mx-auto space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">Payments made to suppliers.</p>
        </div>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {groups.map((g) => (
                  <tr key={g.receiptKey}>
                    ...
                    <td className="px-3 py-2 text-right font-bold text-red-600">₹{g.totalAmount.toLocaleString('en-IN')}</td>
