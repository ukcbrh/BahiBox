...
export function SaleOutstandingView({
 onBack }
: {
 onBack?: () => void }
) {
...
;
      ...
      const computed = (customers || []).map((c: any) => {
        ...
        return {
 ...c, totalInvoiced, totalPaid, outstanding }
;
        ...
        <CardContent className="p-4 flex flex-wrap gap-4 items-end">
          <div className="flex-1 min-w-[200px] space-y-1">
            ...
            <div className="relative">
              ...
              <Input placeholder="Customer name..." className="pl-9" value={
searchTerm}
 onChange={
...
loading ? (
                <tr><td colSpan={
5}
 className="px-6 py-8 text-center text-slate-500">Loading report...</td></tr>
              ) : filteredRows.length === 0 ? (
                <tr><td colSpan={
5}
 className="px-6 py-8 text-center text-slate-500">No outstanding dues found.</td></tr>
              ) : (
...
r.id}
 className="border-b hover:bg-slate-50 dark:hover:bg-slate-900/50">
                    <td className="px-6 py-4 font-medium text-slate-900 dark:text-slate-100">
                      <div className="flex items-center"><User className="h-4 w-4 mr-2 text-slate-400" />{
r.customer_name}
</div>
                    </td>
...
r.totalInvoiced.toLocaleString('en-IN')}
</td>
                    <td className="px-6 py-4 text-right text-slate-600 dark:text-slate-400">₹{
