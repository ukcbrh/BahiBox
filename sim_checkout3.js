...
async function run() {
  const { data: oData } = await supabase.from('orders').select('user_id').limit(1);
  const realUserId = oData[0].user_id;
  ...
  const { data: stockData } = await supabase.from('product_stock').select('current_quantity').eq('product_id', '2a78e779-c41b-4b5a-a032-07146c0bdb03').eq('branch_id', bId).single();
  console.log("New stock:", stockData);
}
run();
